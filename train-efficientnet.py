...
parser = argparse.ArgumentParser(description='Train EfficientNetB7')
...
if args.init_weights:  
    core = EfficientNetB7(include_top=False, weights='imagenet')
else:
    core = EfficientNetB7(include_top=False, weights=None)
core.trainable = not args.init_weights
x = core(x)
...
model.compile(optimizer=keras.optimizers.Adam(lr=args.learning_rate),
...
checkpoint_dir= './enb7_{runid}_{iso}'.format(runid=args.runid,iso=now.strftime("%Y%m%d_%H%M%S"))
